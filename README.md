...
Next.jsはReact 19系をpeer dependencyで許容する16.3.4を維持し、`experimental.viewTransition`やルーター統合は使用しません。

対象は既存Client Component `CartContent`内の小計だけです。Zustandの更新を`startTransition`で囲むだけでは外部storeの同期更新を非同期化できないため、
