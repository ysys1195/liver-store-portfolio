...
export function CartContent() {
  ...
  const removedProductNames = useCartStore(
  ...
  useEffect(() => {
    ...
    const soldOutItems = items.filter((_, index) => {
      const inventory = inventoryQueries[index]?.data;
      return inventory?.stock === 0 || inventory?.status === "sold_out";
    });

    if (soldOutItems.length === 0) return;

    removeSoldOutItems(soldOutItems);
  }, [hasHydrated, inventoryQueries, items, removeSoldOutItems]);
